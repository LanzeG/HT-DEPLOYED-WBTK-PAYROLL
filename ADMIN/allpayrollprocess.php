...
$generatedQuery =   $_SESSION['processallpayroll'];
			...
			$secondhalf = $totalnet - $firsthalf;
						...
						if ($searchrows >=1){
							...
							<script>
								...
								$savepayrollquery = "INSERT INTO pay_per_period (emp_id,pperiod_range,pperiod_month,pperiod_year,rate_per_hour, reg_pay, undertimehours, absences,net_pay, firsthalf, secondhalf) VALUES 
																					('$payID','$payperiodrange','$month','$year','$rph','$salary', '$totalut', '$totalabsences', '$totalnet', '$firsthalf', '$secondhalf')";
								$savepayrollexecquery = mysqli_query($conn,$savepayrollquery) or die ("FAILED TO INSERT PAYROLL INFO ".mysqli_error($conn));
	...
	<script>
		document.addEventListener('DOMContentLoaded', function() {
			swal({
				...
				window.location.href = 'adminPAYROLLProcess.php'; // Replace 'your_new_page.php' with the actual URL
				window.close()
			});
		});
	</script>
...
?>
	<script>
		document.addEventListener('DOMContentLoaded', function() {
			swal({
				text: "Payroll has already been processed",
				con: "success",
				button: "OK",
			}).then(function() {
